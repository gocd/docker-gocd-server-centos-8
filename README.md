...
```shell
docker run -d -p8153:8153 gocd/gocd-server-centos-8:v20.8.0
```
...
```shell
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go gocd/gocd-server-centos-8:v20.8.0
```
...
docker run \
  ...
  gocd/gocd-server-centos-8:v20.8.0
```
...
```shell
docker run -v /path/to/your/script.sh:/docker-entrypoint.d/your-script.sh ... gocd/gocd-server-centos-8:v20.8.0
```

If you have several scripts in a directory that you'd like to execute:

```shell
docker run -v /path/to/script-dir:/docker-entrypoint.d ... gocd/gocd-server-centos-8:v20.8.0
```
...
```shell
docker run -e GOCD_SERVER_JVM_OPTS="-Xmx4096mb -Dfoo=bar" gocd/gocd-server-centos-8:v20.8.0
```
...
```shell
docker run --name server -it -p8153:8153 gocd/gocd-server-centos-8:v20.8.0
```
